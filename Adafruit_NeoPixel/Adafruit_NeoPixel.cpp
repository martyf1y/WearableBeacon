...
uint8_t Adafruit_NeoPixel::getRedColor(uint16_t n) const {
    if(n >= numLEDs) {
        // Out of bounds, return no color.
        return 0;
    }
    uint8_t *p = &pixels[n * 3];
    uint8_t c = ((uint32_t)p[rOffset]);
;
...
// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint8_t Adafruit_NeoPixel::getGreenColor(uint16_t n) const {
    if(n >= numLEDs) {
        // Out of bounds, return no color.
        return 0;
    }
    uint8_t *p = &pixels[n * 3];
    uint8_t c = ((uint32_t)p[gOffset]);
    ;
    // Adjust this back up to the true color, as setting a pixel color will
    // scale it back down again.
    if(brightness) { // See notes in setBrightness()
...
// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint8_t Adafruit_NeoPixel::getBlueColor(uint16_t n) const {
    if(n >= numLEDs) {
        // Out of bounds, return no color.
        return 0;
    }
    uint8_t *p = &pixels[n * 3];
    uint8_t c = (uint32_t)p[bOffset];
    ;
    // Adjust this back up to the true color, as setting a pixel color will
    // scale it back down again.
    if(brightness) { // See notes in setBrightness()
